...
export default function BoardPage() {
  ...
  const seatCount   = (g) => Math.max(getSeats(g).length, 1);
  const upBoarded   = upGuests.filter(g => isBoarded(g, "상행")).reduce((s, g) => s + seatCount(g), 0);
  const downBoarded = downGuests.filter(g => isBoarded(g, "하행")).reduce((s, g) => s + seatCount(g), 0);
  const upTotal     = upGuests.reduce((s, g) => s + seatCount(g), 0);
  const downTotal   = downGuests.reduce((s, g) => s + seatCount(g), 0);
